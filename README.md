# 🌐 G3-Architects-2.0

A modern, fully responsive architectural landing page built using **pure HTML & CSS** — no JavaScript!  
This project focuses on clean design, fast performance, and pixel-perfect responsiveness across all devices.

🔗 **Live Website:** [tanvirjem2.github.io/G3-Architects-2.0](https://tanvirjem2.github.io/G3-Architects-2.0/)

---

## 📸 Preview

![image alt](https://github.com/tanvirjem2/G3-Architects-2.0/blob/7514c63d73da6b0c123976113f01feefc829f4ad/G3_Architects_1.png)
![image alt](https://github.com/tanvirjem2/G3-Architects-2.0/blob/7514c63d73da6b0c123976113f01feefc829f4ad/G3_Architects_3.png)
![image alt](https://github.com/tanvirjem2/G3-Architects-2.0/blob/7514c63d73da6b0c123976113f01feefc829f4ad/G3_Architects_4.png)
![image alt](https://github.com/tanvirjem2/G3-Architects-2.0/blob/7514c63d73da6b0c123976113f01feefc829f4ad/G3_Architects_6.png)

---

## 🚀 Key Features

- 📱 **Fully Responsive Design** (Mobile → Tablet → Desktop)  
- 🎨 **Modern UI/UX** layout  
- ⚡ **Super-fast performance** (no JS)  
- 🧩 **Reusable, organized CSS components**  
- 📄 **Semantic HTML5 markup**  
- 🌍 **Cross-browser compatibility**  

---

## 🛠️ Technologies Used
- **HTML5**
- **CSS3** (Flexbox, Grid, Media Queries)

---

## ⭐ Support the Project

If you like this project, please consider giving it a **⭐ on GitHub** — your support helps a lot!

---
